...
GenerateQBiCScoresFromTwelvemers <- function(uPBM_QBiC_scores,twelvemers.list) {
  ...
  uPBM_QBiC_scores <- uPBM_QBiC_scores[which(!is.na(uPBM_QBiC_scores$z_score)),]
  uPBM_QBiC_scores <- uPBM_QBiC_scores$z_score
...
#'
#'This function generates a mutation spectrum of 96 mutation classes (mutations on pyrimidine centered trinucleotide) from a list of twelvemers
#'
#'@param twelvemers.list A list of twelvemers, with a 11mer centered at the mutation base and 1 mutated base appended
#'
...
#'
#'SignatureQBiC model. This function generates Gain Ratio and Loss Ratio for a TF (represented by universal PBM) with
...
SignatureQBiC <- function(uPBM_QBiC_scores, spectrum) {
  uPBM_QBiC_scores <- data.frame(uPBM_QBiC_scores)
  uPBM_QBiC_scores <- uPBM_QBiC_scores[which(!is.na(uPBM_QBiC_scores$z_score)),]
  PBM.scores <- data.frame(uPBM_QBiC_scores$z_score)
